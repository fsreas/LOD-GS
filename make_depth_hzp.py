...
if __name__ == "__main__":
    ...
    # hzp added, for ref pointcloud
    # ref_pcd = fetchPly("/data/new_disk5/hezhpl/outdoor/LetsGo/point_cloud/iteration_300000/point_cloud.ply")
    # gaussians.create_from_pcd(ref_pcd, cameras_extent)
    # end modification
    ...
    for _id, cam_info in enumerate(scene_info.train_cameras):
        ...
        camera = Camera(colmap_id=cam_info.uid, R=cam_info.R, T=cam_info.T, 
                  FoVx=cam_info.FovX, FoVy=cam_info.FovY, cx=orig_w / 2.0, cy = orig_h / 2.0,
                  image=gt_image, gt_alpha_mask=loaded_mask,
                  image_name=cam_info.image_name, depth=None, uid=_id, data_device=args.data_device)
    ...
    with torch.no_grad():
        for camera in tqdm(camera_list):
            render_pkg = render(camera, gaussians, pipe=pipe, bg_color=background)
